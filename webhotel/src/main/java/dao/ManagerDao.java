...
public class ManagerDao {
	...
	public static HotelListInfo printReport(int staffId) {
		...
		try {
			con = Conn.getConnection();
			stmt = con.createStatement();
			res = stmt.executeQuery(String.format("select hotel_id,hotel_area,address,sum(bill_fee) from hotel_has_staff natural join bill natural join hotel where staff_id=%d and charge_state=1 group by hotel_id", staffId));
			while (res.next()) {
	...
	public static CheckMsg printBill(int hotelId) {
		...
		try {
			con = Conn.getConnection();
			stmt = con.createStatement();
			res = stmt.executeQuery(String.format("select * from bill where hotel_id=%d", hotelId));
			ckm=new CheckMsg();
			while (res.next()) {
