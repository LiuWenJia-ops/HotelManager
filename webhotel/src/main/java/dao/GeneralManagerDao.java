...
public class GeneralManagerDao {
	...
	public static AreaReport printAreaReport() {
		...
		try {
			...
			res = stmt.executeQuery(String.format(
					"select sum(bill_fee),hotel_area from bill natural join hotel where charge_state=1 group by hotel_area"));
			while (res.next()) {
	...
	public static AddressReport printAddressReport(String area) {
		...
		try {
			...
			stmt = con.createStatement();
			res = stmt.executeQuery(String.format(
					"select hotel_id,address,sum(bill_fee) from bill natural join hotel where charge_state=1 and hotel_area='%s' group by address",area));
			while (res.next()) {
