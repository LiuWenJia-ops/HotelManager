...
public class ReceptionDao {
	...
	public static List<String> watchCheckinBill(int customerId) {
		...
		try {
			con = Conn.getConnection();
			stmt = con.createStatement();
			res = stmt.executeQuery(String.format("select * from bill where customer_id=%d and is_in=0 and can_delete=1", customerId));
			while (res.next()) {
				// 返回订单列表（其实只有一个）
	...
	public static boolean checkIn(int billId) {
		...
		try {
			...
			con = Conn.getConnection();
			stmt = con.createStatement();
			stmt.executeUpdate(String.format("update bill set date_in='%s',can_delete=0,is_in=1 where bill_id=%d",
	...
	public static List<String> watchCheckoutBill(int customerId) {
		...
		try {
			con = Conn.getConnection();
			stmt = con.createStatement();
			res = stmt.executeQuery(String.format("select * from bill where customer_id=%d and is_in=1 and can_delete=0", customerId));
			while (res.next()) {
				// 返回订单列表（其实只有一个）
				list.add(res.getString(1));
				list.add(res.getString(3));
	...
	public static boolean checkOut(int billId) {
		...
		try {
			con = Conn.getConnection();
			stmt = con.createStatement();
			res = stmt.executeQuery(String.format("select hotel_id,room_type from bill where bill_id=%d", billId));
			while (res.next())
			{
			...
			// 前端显示“客户已成功退房”
			stmt.executeUpdate(String.format("update bill set is_in=0 where bill_id=%d",billId));
			Conn.close(con, stmt, res);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
